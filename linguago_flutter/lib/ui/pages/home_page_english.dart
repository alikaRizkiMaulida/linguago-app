...
import 'package:linguago_flutter/ui/script/pages/english_script_page.dart';
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Text(
                      QuizProgress.unlockedPart <= 5 ? 'Level 1' : 'Level 2',
                        ...
                        onTap: () {
                          ...
                          if (unlocked == 1) {
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 7),
                              ),
                            );
                          } else if (unlocked == 6) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 8),
                              ),
                            );
                          } else if (unlocked == 7) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 9),
                              ),
                            );
                          } else if (unlocked == 8) {
                            Navigator.push(
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 11),
                              ),
                            );
                          } else if (unlocked == 11) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 12),
                              ),
                            );
                          } else if (unlocked == 12) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 13),
                              ),
                            );
                          } else if (unlocked == 13) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 14),
                              ),
                            );
                          } else if (unlocked == 14) {
                            Navigator.push(
                              context,
                              MaterialPageRoute<void>(
                                builder: (_) => const QuizScreen(part: 15),
                              ),
                            );
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('All levels completed! Great job!'),
...
class _LevelMapCardEnState extends State<_LevelMapCardEn>
  ...
  int _getActiveNodeIndex(int unlocked) {
    if (unlocked == 1) return 0;
    if (unlocked >= 2 && unlocked <= 4) return 1;
    if (unlocked >= 5 && unlocked <= 7) return 2;
    if (unlocked == 8) return 3;
    if (unlocked >= 9 && unlocked <= 11) return 4;
    if (unlocked >= 12 && unlocked <= 14) return 5;
    return -1;
  }

  bool _isNodeLocked(int nodeIndex, int unlocked) {
    if (nodeIndex == 0) return unlocked < 1;
    if (nodeIndex == 1) return unlocked < 2;
    if (nodeIndex == 2) return unlocked < 5;
    if (nodeIndex == 3) return unlocked < 8;
    if (nodeIndex == 4) return unlocked < 9;
    if (nodeIndex == 5) return unlocked < 12;
    return true;
  }
        ...
        children: [
          ...
          Row(
            ...
            children: List.generate(totalLevels, (i) {
              final levelNum = i + 1;
              final int activeIndex = _getActiveNodeIndex(currentLevel);
              final bool isActive = i == activeIndex;
              final bool isLocked = _isNodeLocked(i, currentLevel);
              ...
              Text(
                currentLevel == 1
                    ...
                    : (currentLevel >= 2 && currentLevel <= 4)
                        ? '330/1000'
                        : (currentLevel >= 5 && currentLevel <= 7)
                            ? '500/1000'
                            : currentLevel == 8
                                ? '660/1000'
                                : (currentLevel >= 9 && currentLevel <= 11)
                                    ? '830/1000'
                                    : '1000/1000',
                style: const TextStyle(
                    fontSize: 12,
